...
!(function(a, b) {
  ...
  var a = {
    ...
    createBackend: function() {
      ...
      this.backend && this.backend.destroy(),
        ...
        this.backend.on("audioprocess", function(a) {
          b.drawer.progress(b.backend.getPlayedPercents()),
    ...
    loadBuffer: function(a, b) {
      ...
      return b
        ? (this.backend.setPeaks(b),
          this.drawBuffer(),
          this.tmpEvents.push(this.once("interaction", c)),
          void 0)
        : c();
    },
  ...
  return (
    ...
    a.util.extend(a.MediaElement, {
      ...
      loadElt: function(a, b) {
        var c = a;
        (c.controls = this.params.mediaControls),
          (c.autoplay = this.params.autoplay || !1),
          this._load(c, b);
      },
      _load: function(a, b) {
        var c = this;
      ...
      handleEvent: function(a, b) {
        ...
        return (
          !this.params.fillParent && f < g
            ? ((c = ((d - e.left) * this.params.pixelRatio) / f || 0),
              c > 1 && (c = 1))
            : (c =
                (d - e.left + this.wrapper.scrollLeft) /
      ...
      drawBars: function(b, c) {
        if (b[0] instanceof Array) {
          ...
          if (this.params.splitChannels)
            return (
              this.setHeight(
                d.length * this.params.height * this.params.pixelRatio
              ),
        ...
        e &&
          (b = [].filter.call(b, function(a, b) {
            return b % 2 == 0;
          }));
        var f = 0.5 / this.params.pixelRatio,
          g = this.width,
      ...
      drawWave: function(b, c) {
        ...
        if (!e) {
          for (var f = [], g = 0, h = b.length; g < h; g++)
            (f[2 * g] = b[g]), (f[2 * g + 1] = -b[g]);
          b = f;
        }
        var i = 0.5 / this.params.pixelRatio,
    ...
    a.util.extend(a.Drawer.MultiCanvas, {
      initDrawer: function(a) {
        if (
          ((this.maxCanvasWidth =
            null != a.maxCanvasWidth ? a.maxCanvasWidth : 4e3),
      ...
      updateSize: function() {
        for (
          var a = Math.round(this.width / this.params.pixelRatio),
            b = Math.ceil(a / this.maxCanvasElementWidth);
        ...
        for (var c in this.canvases) {
          ...
          c == this.canvases.length - 1 &&
            (d = this.width - this.maxCanvasWidth * (this.canvases.length - 1)),
      ...
      removeCanvas: function() {
        ...
        a.wave.parentElement.removeChild(a.wave),
          this.hasProgressCanvas &&
            a.progress.parentElement.removeChild(a.progress);
      },
      ...
      clearWaveForEntry: function(a) {
        a.waveCtx.clearRect(
          0,
          0,
          a.waveCtx.canvas.width,
          a.waveCtx.canvas.height
        ),
          this.hasProgressCanvas &&
            a.progressCtx.clearRect(
              0,
              0,
              a.progressCtx.canvas.width,
              a.progressCtx.canvas.height
            );
      },
      drawBars: function(b, c) {
        if (b[0] instanceof Array) {
      ...
      drawWave: function(b, c) {
        ...
        this.drawLine(b, l, k, j),
          this.fillRect(0, k + j - this.halfPixel, this.width, this.halfPixel);
      },
      drawLine: function(a, b, c, d) {
        for (var e in this.canvases) {
          var f = this.canvases[e];
          this.setFillStyles(f),
            this.drawLineToContext(f, f.waveCtx, a, b, c, d),
        ...
        for (var e in this.canvases) {
          var f = this.canvases[e],
            ...
            h = {
              ...
              x2: Math.min(
                a + c,
                e * this.maxCanvasWidth + f.waveCtx.canvas.width
              ),
              y2: b + d
            };
          h.x1 < h.x2 &&
      ...
      setFillStyles: function(a) {
        (a.waveCtx.fillStyle = this.params.waveColor),
          this.hasProgressCanvas &&
            (a.progressCtx.fillStyle = this.params.progressColor);
      },
      updateProgress: function(a) {
        var b = Math.round(this.width * a) / this.params.pixelRatio;
        this.style(this.progressWave, { width: b + "px" });
